...
module.exports = function(config) {
    ...
    const peerConfig1 = config.tlsEnabled
        ...
        : null;
    const peer1 = fabric_client.newPeer(
        ...
        invoke(user, invokeRequest) {
            ...
            return this.get_member_user(user)
                .then( async user_from_store =>{
                    ...
                    var arrArg = await [tx_id.getTransactionID()];
                    for (var j = 0; j < invokeRequest.args.length; j ++) {
                        arrArg.push(invokeRequest.args[j]);
                    }

                    console.error("arrArg:", arrArg);


                    let results1 = await channel.sendTransactionProposal({
                    ...
                    // var fs = require('fs');
                    // config.peerPem = fs.readFileSync('/home/luatcoi/Project/src/driving-files/crypto-config/peerOrganizations/org1/peers/peer0.org1/tls/ca.crt');
                    // config.peerDomain = "peer0.org1";
                    // config.peerHost = "13.229.68.163:30501";

                    // const peerConfig2 = config.tlsEnabled
                    // ? {
                    //     pem: Buffer.from(config.peerPem).toString(),
                    //     "ssl-target-name-override":
                    //     config.peerDomain || config.peerHost.split(":")[0]
                    // }
                    // : null;
                    // const peer2 = fabric_client.newPeer(
                    //     grpcProtocol + config.peerHost,
                    //     peerConfig2
                    // );
                    ...
                    // let results2 = await channel.sendTransactionProposal({
                    //     chaincodeId: invokeRequest.chaincodeId,
                    //     fcn: invokeRequest.fcn,
                    //     args: arrArg,
                    //     txId: tx_id
                    ...
                    var proposalResponses1 = results1[0];
                    var proposal1 = results1[1];
                    // var proposalResponses2 = results2[0];
                    // var proposal2 = results2[1];
                    let isProposalGood = false;

                    if (
                        proposalResponses1 &&
                        proposalResponses1[0].response &&
                        proposalResponses1[0].response.status === 200 
                        // proposalResponses2 &&
                        // proposalResponses2[0].response &&
                        // proposalResponses2[0].response.status === 200
                    ) {
                        isProposalGood = true;
                        // console.log("Transaction proposal was good");
                    } else {
                        console.error("Transaction proposal was bad");
                        console.error("results1:", results1);
                        console.error("proposal1:", proposal1);
                        console.error("proposalResponses1:", proposalResponses1);
                        console.error(proposalResponses1[0].response);
                        console.error(proposalResponses1[0].response.status);

                        // console.error("results2:", results2);
                        // console.error("proposal2:", proposal2);
                        // console.error("proposalResponses2:", proposalResponses2);
                        // console.error(proposalResponses2[0].response);
                        // console.error(proposalResponses2[0].response.status);
                    }
                    ...
                    if (isProposalGood) {
                        ...
                        const sendPromise = channel.sendTransaction({
                            proposalResponses: proposalResponses1,
                            proposal: proposal1
                        });

                        console.error("sendPromise:", sendPromise);
